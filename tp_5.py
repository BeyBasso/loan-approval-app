...
st.pyplot(fig) #Afficher dans Steamlit

import plotly.express as px

fig = px.histogram(df, x='ApplicantIncome', title='Distribution des revenus')
st.plotly_chart(fig, use_container_width=True)

#Visualisation essentielles avec  Plotly
##1) Histogramme(Distribution)
fig = px.histogram(
    df,
    x='ApplicantIncome',
    title='Distribution des revenus',
    labels={'ApplicantIcome': 'Revenu (€)'},
    color_discrete_sequence=['#636EFA']
)
st.plotly_chart(fig, use_container_width=True)
